...
SELECT e.id,
       ...
       (
           3 * (SELECT COUNT(*) FROM event_attendees ea WHERE ea.event_id = e.id)
               + (SELECT COUNT(*) FROM saved_events se WHERE se.event_id = e.id)
           )                      AS popularity_score,
       e.is_cancelled,
       e.is_published,
       'app'::TEXT                AS source,
...
SELECT ext.event_id,
       ...
       (
           3 * (SELECT COUNT(*) FROM event_attendees ea WHERE ea.event_id = ext.event_id)
               + (SELECT COUNT(*) FROM saved_events se WHERE se.event_id = ext.event_id)
           )                              AS popularity_score,
       ext.is_cancelled,
